    ...
    <div className="bg-white max-sm:min-w-[50vw]  max-h-fit rounded">
      <div className="p-2">
        ...
        <img
          ...
          className="mx-auto object-contain cursor-pointer rounded border-2 p-1 hover:border-green-400"
          onClick={() => {
            setFullImage(true);
            setFullImageUrl(
              data?.productImage?.[0] || toast.error("Image not available.")
            );
          }}
          style={{ width: "120px", height: "120px" }} // Inline styles to fix the size
        />

        {/* Product Name */}
        <h1 className="text-center text-ellipsis line-clamp-1 font-medium sm:text-md mt-2">
          {data?.productName}
        </h1>
