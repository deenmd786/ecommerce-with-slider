...
function AdminEditProduct({ onClose, importData, fetchFuc }) {
  const [data, setData] = useState({
    _id: importData?._id,
    productName: importData?.productName,
    brandName: importData?.brandName,
    category: importData?.category,
    productImage: importData?.productImage || [],
    description: importData?.description,
    price: importData?.price,
    selling: importData?.selling,
  });
  ...
  const handleUploadProduct = async ({ target: { files } }) => {
    ...
    setData((prevData) => ({
      ...prevData,
      productImage: [...prevData.productImage, url],
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetchData(
        `/product/update-product/${importData._id}`,
        'POST',
        data,
        true
      );
  ...
  return (
    <div className="fixed w-full h-full bg-slate-200 bg-opacity-35 inset-0 flex justify-center items-center z-20 max-sm:px-3">
      <div className="bg-white p-4 sm:p-6 rounded-lg w-full max-w-2xl max-h-[80%] overflow-hidden shadow-lg">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-2 sm:h-20 max-sm:flex-col">
              ...
              <div className="flex gap-2 sm:overflow-x-auto">
              {data.productImage.map((imgUrl, index) => (
                <div key={index} className="relative shrink-0">
                  {loadingStates[index] && <SingleImageShimmer />}
...
AdminEditProduct.propTypes = {
  ...
  importData: PropTypes.shape({
    ...
    selling: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
  }),
};
